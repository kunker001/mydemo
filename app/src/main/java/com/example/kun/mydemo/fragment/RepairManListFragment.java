...
public class BaoxiujiluFragment extends BaseFragment {
    ...
    protected int getLayoutId() {
        return R.layout.fragment_baoxiujilu;
    }
    ...
    private void initRecyView() {
        try {
            ...
            mAdapter = new MyRecyAdapter(arrayBx,0);
    ...
    public static BaoxiujiluFragment newInstance() {
        return new BaoxiujiluFragment();
    }
    ...
    private void turn2(ContentBean cb){
        for (ContentBean.DataBean bean : cb.getContent()) {
            ...
            try {
                if(type == 0 && ( bean.getRepair_status() == 3 || bean.getRepair_status() == 4) ){
                    continue;  //过滤评价
                }
                if(type == 1 && bean.getRepair_status() < 3){
                    continue;  //过滤评价
                }
                arrayBx.add(new Baoxiu(bean.getContent(),bean.getAddress(),bean.getIsEvaluate(),
                        ...
                        bean.getTelephone(),bean.getArea(),bean.getId(),bean.getRepair_status()));
    ...
    private void getDataList( int page ) {
        DataReqBean dq = new DataReqBean("","","","",10,page,SpMap.gEt("sno"),"");
